...
const emailCheck = (email) => {
  const convertEmail = String(email).trim();
  if (convertEmail.includes('@')) {
...
const buyFrogs2 = (gold) => {
  let frogPrice = 3;
  let frogsPurchased = Math.floor(gold / frogPrice);
  return frogsPurchased
}
...
let isAscending = ascendingArray(sampleArray);
